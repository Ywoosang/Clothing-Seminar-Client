      ...
      <section class="register">
        <div class="input-wrapper">
          ...
          <textarea
            v-model="content"
            class="content"
            placeholder="학술대회에 대한 의견들을 자유롭게 남겨주세요."
          ></textarea>
        </div>
        <button class="submit-review" @click="postReview">
          후기<br />작성
        </button>
      </section>
...
export default {
  ...
  setup() {
    ...
    const setPageNumber = () => {
      ...
      const numberOfPages =
        Math.floor(tmpNumber) == tmpNumber
          ? Math.floor(tmpNumber)
          : Math.floor(tmpNumber) + 1;
      pages.value = Array(numberOfPages || 1)
        .fill(0)
    ...
    const deleteReview = async (index: number) => {
      try {
        let clientPassword, response;
        const isAdmin =
          store.state.authority == "ROOT" ||
          store.state.authority == "ADMINISTER";
        if (isAdmin) {
          const isDelete = confirm(
            "정말로 해당 댓글을 관리자 권한으로 삭제하시겠습니까?"
          );
          if (!isDelete) return;
          response = await request.deleteReviewByAdminRole({
            reviewId: totalReviews.value[index].id,
          });
        } else {
          clientPassword = prompt("비밀번호를 입력해 주세요");
          response = await request.deleteReviewByPassword({
            reviewId: totalReviews.value[index].id,
